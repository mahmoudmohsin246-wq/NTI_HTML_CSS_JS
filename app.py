...
from part_2 import courses, Course
from Part_3 import convert_to_gpa
import part_4
...
def update_student(student_id):
    ...
    # Update dictionary reference inside student object
...
def delete_student(student_id):
    ...
    # Remove student from course grade listings if present
...
def get_student_gpa(student_id):
    ...
    for course in courses:
        # check if student has a grade recorded by student ID or student name
        raw_grade = None
        if student_id in course.students:
...
def register_grade(course_id):
    ...
    # Store grade under student_id (or student_name if specified)
...
def save_database():
    try:
        part_4.save_students()
        return jsonify({"status": "success", "message": "Database saved successfully to student.json."})
...
def load_database():
    try:
        part_4.load_students()
        return jsonify({
            "status": "success",
            "message": f"Database loaded successfully! Total loaded students: {len(students)}.",
            "total_students": len(students)
        })
    except Exception as e:
...
if __name__ == "__main__":
    # Pre-load database if student.json exists
    if os.path.exists(part_4.STUDENT_JSON):
        try:
            part_4.load_students()
            print(f"[Init] Pre-loaded {len(students)} students from student.json")
        except Exception as e:
            print(f"[Init] Could not pre-load student.json: {e}")
